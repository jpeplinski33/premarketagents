...
## 9. Invite link preview image (OG prototype — 2026-08-09)
...
On 2026-08-09, Codex verified that the live v4 images were byte-identical to the
local branded 1200×630 JPEGs and that all three listing pages referenced them.
Because Apple still returned a stale card, the same verified art was published
under fresh `*-exclusive-v5.jpg` URLs. Never reuse an OG filename after it has
been exposed to a messaging crawler; update `og:image`, `og:image:secure_url`,
and `twitter:image` together.
